		...
		Resources: []pluginapi.ResourceRoute{
			{
				...
				Description: "Open Codex Fleet Manager quota status.",
			},
		},
...
<div class="cardActions"><button type="button" class="ghost pinAccount {{if eq .SchedulerPriority 999}}pinned{{end}}" data-auth-id="{{.AuthID}}" data-pinned="{{if eq .SchedulerPriority 999}}true{{else}}false{{end}}" aria-pressed="{{if eq .SchedulerPriority 999}}true{{else}}false{{end}}" aria-label="固定账号" title="固定账号"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M15.5 3.5l5 5a2 2 0 0 1-2.8 2.8l-1.2-1.2-4.4 4.4.7 2.8-1.4 1.4-6.1-6.1 1.4-1.4 2.8.7 4.4-4.4-1.2-1.2a2 2 0 0 1 2.8-2.8Z"/><path d="M8.5 15.5 3 21"/></svg></button><button type="button" class="ghost refreshOne" data-auth-id="{{.AuthID}}">刷新额度</button><button type="button" class="secondary openEdit" data-auth-id="{{.AuthID}}">编辑</button></div>
...
<main class="main settingsPage" id="settingsPage" hidden>
...
<div class="settingsCard retryCPACard"><div class="actions"><button id="retryCheckCPA" type="button" class="ghost" data-i18n="retry.checkCPA">Check &amp; fix CPA retry settings</button></div><div id="retryCPAConfirm" class="retryCPAConfirm" hidden><strong data-i18n="retry.cpaConfirmTitle">Review CPA retry settings</strong><p id="retryCPAConfirmBody"></p><div class="actions"><button id="retryCPAConfirmYes" type="button" data-i18n="retry.cpaConfirmYes">Apply recommended settings</button><button id="retryCPAConfirmNo" type="button" class="ghost" data-i18n="retry.cpaConfirmNo">Cancel</button></div></div><p class="setting-help" id="retryCPAStatus"></p></div>
...
<div class="setting-with-help wide"><p class="setting-help" data-i18n="retry.alwaysHelp">对没有配置模型链的请求也启用重试；没有备用模型时按同一模型再次尝试。与影子模式互斥。</p></div>
...
<script>
let STATUS={{json .}};
const MANAGEMENT_BASE='/v0/management/plugins/codex-fleet-manager';
const LOCALE_STORAGE_KEY='codex-fleet-manager-locale-v1';
...
const resetProbeWarning=document.getElementById('resetProbeWarning');
...
function createPinButton(account){const pinned=Number(account.scheduler_priority)===999;const button=node('button','ghost pinAccount '+(pinned?'pinned':''),'');button.type='button';button.dataset.authId=account.auth_id||'';button.dataset.pinned=String(pinned);button.setAttribute('aria-pressed',String(pinned));const label=t(pinned?'actions.unpinAccount':'actions.pinAccount');button.setAttribute('aria-label',label);button.title=label;button.innerHTML='<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M15.5 3.5l5 5a2 2 0 0 1-2.8 2.8l-1.2-1.2-4.4 4.4.7 2.8-1.4 1.4-6.1-6.1 1.4-1.4 2.8.7 4.4-4.4-1.2-1.2a2 2 0 0 1 2.8-2.8Z"/><path d="M8.5 15.5 3 21"/></svg>';button.addEventListener('click',()=>togglePin(account.auth_id||'',pinned,button));return button}
...
async function exportConfig(){try{const data=await requestManagement('/export');const blob=new Blob([JSON.stringify(data,null,2)+'\n'],{type:'application/json'});const link=document.createElement('a');link.href=URL.createObjectURL(blob);link.download='codex-fleet-manager-config.json';link.click();URL.revokeObjectURL(link.href);showNotice(t('notice.configExported'),false,'notice.configExported');await refreshLogs()}catch(error){showNotice(error.message||String(error),true)}}
async function exportLogs(){try{await refreshStatus({management:true});const payload={plugin_id:STATUS.plugin_id||'codex-fleet-manager',exported_at:new Date().toISOString(),logs:STATUS.logs||[]};const blob=new Blob([JSON.stringify(payload,null,2)+'\n'],{type:'application/json'});const link=document.createElement('a');link.href=URL.createObjectURL(blob);link.download='codex-fleet-manager-logs.json';link.click();URL.revokeObjectURL(link.href);showNotice(t('notice.logsExported'),false,'notice.logsExported')}catch(error){showNotice(error.message||String(error),true)}}
...
queueNav.addEventListener('click',()=>showPage('queue',true));
settingsNav.addEventListener('click',()=>showPage('settings',true));
document.getElementById('settingsBack').addEventListener('click',()=>showPage('queue',true));
window.addEventListener('hashchange',()=>{if(statusLoaded)showPage(requestedPage(),false)});
